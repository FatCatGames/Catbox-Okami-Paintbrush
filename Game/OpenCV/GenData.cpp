...
//
//void ReadImageSVM(Ptr<SVM> svm, cv::Mat anImage, BrushSymbol& outSymbol, const std::function<void(bool success)>& aCallback)
//{
//
...
//
//	cv::Mat matTestingFloat;
//	cv::resize(matThresh, matTestingFloat, cv::Size(RESIZED_IMAGE_WIDTH, RESIZED_IMAGE_HEIGHT));
//	cv::Mat matTestingFlattenedFloat = matTestingFloat.reshape(1, 1);
//
//
//
...
//		imshow("Input", matROIResized);
//		cv::waitKey(0);
//		float fltCurrentChar = svm->predict(anImage);
//
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void ReadImagekNN(cv::Mat anImage, BrushSymbol& outSymbol, const std::function<void(bool success)>& aCallback)
{
...
void GenData::GetSymbol(BrushSymbol& anOutSymbol, ID3D11Texture2D* aTexture, int width, int height, const std::function<void(bool aSucceeded)>& aCallback)
{
	...
	//Engine::GetInstance()->GetThreadPool()->QueueJob([&] { ReadImageSVM(mat, svm); });
}
